...
updateAutoSendUI();
            ...
            const updateAutoEnvelopeUI = () => {
                if (!autoEnvelopeToggle) return;
                ...
                if (autoEnvelopeSlider) autoEnvelopeSlider.value = currentVal;
                if (autoEnvelopeValue) autoEnvelopeValue.textContent = `${currentVal}小时`;
            };

            if (autoEnvelopeToggle) {
                autoEnvelopeToggle.addEventListener('click', () => {
                    settings.autoEnvelopeEnabled = !settings.autoEnvelopeEnabled;
                    updateAutoEnvelopeUI();
                    manageAutoEnvelopeTimer();  // 注意这个函数在 core.js 里
                    throttledSaveData();
                    showNotification(`梦角主动寄信已${settings.autoEnvelopeEnabled ? '开启' : '关闭'}`, 'success');
                });
            }

            if (autoEnvelopeSlider) {
                // 初始化滑块值（注意浮点数处理）
                autoEnvelopeSlider.value = settings.autoEnvelopeInterval || 5;
                if (autoEnvelopeValue) autoEnvelopeValue.textContent = `${settings.autoEnvelopeInterval || 5}小时`;
                autoEnvelopeSlider.addEventListener('input', (e) => {
                    // 使用 parseFloat 支持小数（0.5 等）
                    const val = parseFloat(e.target.value);
                    settings.autoEnvelopeInterval = val;
                    if (autoEnvelopeValue) autoEnvelopeValue.textContent = `${val}小时`;
                });
                autoEnvelopeSlider.addEventListener('change', () => {
                    manageAutoEnvelopeTimer();
                    throttledSaveData();
                });
            }

            // 最后在某个初始化 UI 的地方（比如 updateDelayUI 函数后面）调用一次 updateAutoEnvelopeUI();
            // 可以在 updateAutoSendUI() 后面加一句：
            updateAutoEnvelopeUI();
