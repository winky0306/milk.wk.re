...
// 主动寄信内容生成：直接复用回信生成函数，逻辑与回信完全一致
function generateAutoEnvelopeContent() {
    // 如果字库为空，返回空字符串，由 sendAutoEnvelope 处理提示语
    if (typeof customReplies === 'undefined' || !customReplies || customReplies.length === 0) {
        return '';
    }
    // 调用现有的回信生成函数（该函数从 customReplies 中抽取 8～12 个句子）
    return generateEnvelopeReplyText();
}
